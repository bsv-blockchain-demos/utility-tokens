...
const main = async () => {
    ...
    server.configureLogger(console)
    server.configureEngineParams({ logTime: true })
    ...
    if (process.env.CHAINTRACKS_URL) {
        const ctsc = new ChaintracksServiceClient(process.env.NETWORK as 'main' | 'test', process.env.CHAINTRACKS_URL)
        const chainTracker = new ChaintracksChainTracker(process.env.NETWORK as 'main' | 'test', ctsc);
